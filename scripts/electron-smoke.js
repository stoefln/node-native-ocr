...
function runDirectElectronSmoke() {
	...
	writeUtf8(
		...
		[
			...
			'  try {',
			`    const txt = await mod.recognize(fs.readFileSync(fixture), {lang: 'eng', tessdataPath: ${JSON.stringify(TessdataPath)}, psm: 6, requireNonEmpty: true})`,
...
function runBundledLayoutSmoke() {
	...
	writeUtf8(
		...
		[
			...
			'app.whenReady().then(async () => {',
			'  try {',
			`    const text = await recognize(fs.readFileSync(fixturePath), {lang: 'eng', tessdataPath: ${JSON.stringify(TessdataPath)}, psm: 6, requireNonEmpty: true})`,
