...
function runDirectElectronSmoke() {
	...
	writeUtf8(
		...
		[
			...
			'  try {',
			'    const imageBuffer = nativeImage.createFromPath(fixture).toPNG()',
			"    assert.strictEqual(mod.__internal.getBackendName(), 'native')",
			`    const txt = await mod.recognize(imageBuffer, {lang: 'eng', tessdataPath: ${JSON.stringify(TessdataPath)}, requireNonEmpty: true})`,
...
function runBundledLayoutSmoke() {
	...
	writeUtf8(
		...
		[
			...
			'app.whenReady().then(async () => {',
			'  try {',
			'    const imageBuffer = nativeImage.createFromPath(fixturePath).toPNG()',
			"    assert.strictEqual(__internal.getBackendName(), 'native')",
			`    const text = await recognize(imageBuffer, {lang: 'eng', tessdataPath: ${JSON.stringify(TessdataPath)}, requireNonEmpty: true})`,
